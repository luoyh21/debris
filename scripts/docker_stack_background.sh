...
echo "    顺序：Space-Track → GCAT / UNOOSA / UCS / ESA / Asterank / TechPort → 刷新 v_unified_objects"
docker rm -f debris-ingest 2>/dev/null || true
docker compose run -d --name debris-ingest app sh -c '
  set -e
  echo "=== [1/3] Space-Track ingest (full) ==="
  python run.py ingest
  echo "=== [2/3] All external sources (GCAT + UNOOSA + UCS + ESA + Asterank + TechPort) ==="
  python scripts/ingest_external.py
  echo "=== [3/3] Refresh v_unified_objects ==="
...
echo "  如需按样本量小跑: docker compose run --rm app sh -c \"python run.py ingest --limit 500 && python scripts/ingest_external.py --limit 500 && python run.py refresh-views\""
